...
void main()
{
	...
	{
		gl_Layer = g_draw[0] + uLayerOffset;
...
#endif
		EmitVertex();
		gl_Layer = g_draw[1] + uLayerOffset;
		gl_PrimitiveID = gl_PrimitiveIDIn;
		gl_Position = tmp1;
#ifdef PASS_ON_GEOMETRY_ID
		g_geometry_id = g_geometry[1];
#endif
#ifdef GBUFFER
...
#endif
		EmitVertex();
		gl_Layer = g_draw[2] + uLayerOffset;
		gl_PrimitiveID = gl_PrimitiveIDIn;
		gl_Position = tmp2;
#ifdef PASS_ON_GEOMETRY_ID
		g_geometry_id = g_geometry[2];
#endif
#ifdef GBUFFER
	...
	for(int i = 0; i < 3; i++)
	{
		gl_Layer = g_draw[i] + uLayerOffset;
		gl_PrimitiveID = gl_PrimitiveIDIn;
		gl_Position = gl_in[i].gl_Position;
#ifdef PASS_ON_GEOMETRY_ID
		g_geometry_id = g_geometry[i];
#endif
#ifdef GBUFFER
